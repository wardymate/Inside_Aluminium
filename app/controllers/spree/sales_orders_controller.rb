class Spree::Admin::SalesOrdersController < Spree::Admin::BaseController
  ...
  def create
    ...
    if @sales_order.save
      ...
      @stock_transfer.stock_movements.where(sales_order_id: nil).each do |stock_movement|
        if stock_movement.estimated_arrival_date == @date
          stock_movement.update(sales_order_id: @sales_order.id, initial_ead: @date, status: 'On Order')
        end
      end
  ...
  def update
    ...
    if @stock_transfer.status == 'Confirmed By Stylmark' && @sales_order.status != 'Complete'
      @stock_movements.each do |movement|
        movement.update(status: 'On Order')
      end
    end
  ...
  def destroy
    ...
    if @stock_transfer.stock_movements.all? { |status| status[:status] == 'Pending'}
      @stock_transfer.update(status: 'Sent To Stylmark')
    end
    redirect_to admin_stock_transfer_path(@stock_transfer)
  end
